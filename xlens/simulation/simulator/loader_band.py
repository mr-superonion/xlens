...
class MakeDMExposureSV_SingleBand(SimulateBase):
    def __init__(
        self,
        config_name,
        cat_i,
    ...
    def generate_exposure(self, file_name):
        ...
        print("reading %s band" % self.band)
        # Add noise
        nstd_f = self.base_std[self.band] * self.noise_ratio
        if "deep" in file_name or "deg" in file_name:
            nstd_f *= np.sqrt(self.deep_noise_frac)
            weight = 1.0 / (self.base_std[self.band] * np.sqrt(self.deep_noise_frac)) ** 2.0
        weight = 1.0 / (self.base_std[self.band]) ** 2.0
        variance += (nstd_f * weight) ** 2.0
        print("Using noisy setup with std: %.2f" % nstd_f)
        if nstd_f > 1e-4:
            seed = self.get_seed_from_fname(file_name, self.band)
            ...
            noise_array = 0.0
        star_array = (
            star_outcome["band_data"][self.band][0].getMaskedImage().image.array
        )
        msk_array = msk_array | (
            star_outcome["band_data"][self.band][0].getMaskedImage().mask.array
        )
        gal_array = (
            gal_array
            + (
                fitsio.read(file_name.replace("_xxx", "_%s" % self.band))
                + star_array
                + noise_array
            )
            * weight
        ...
        se_wcs = star_outcome["se_wcs"][self.band][0]
        dm_psf = make_dm_psf(
            psf=psf_obj,
            psf_dim=psf_dim,
            wcs=deepcopy(se_wcs),
