...
def combine_sim_exposures(exposures: Sequence[ExposureF]) -> ExposureF:
    """Combine simulated exposures using inverse-variance weights.

    Parameters
    ----------
    exposures
        Iterable of exposures generated by :class:`MultibandSimTask` that
        should be combined.  The function assumes that the variance plane of
        every exposure is spatially constant, as is the case for the simulator
        output.

    Returns
    -------
    lsst.afw.image.ExposureF
        New exposure with an image plane equal to the inverse-variance
        weighted mean of the inputs and a variance plane equal to the
        corresponding combined variance.  The exposure inherits the metadata
        (WCS, PSF, mask, etc.) from the first element in ``exposures``.

    Notes
    -----
    The routine raises :class:`ValueError` if ``exposures`` is empty, if the
    image shapes differ, or if the variance plane in any exposure is not
    spatially constant or non-positive.
    """

    exposure_list = list(exposures)
    if len(exposure_list) == 0:
        raise ValueError("combine_sim_exposures requires at least one exposure")

    reference_shape = exposure_list[0].getMaskedImage().image.array.shape
    ...
    combined_image = weighted_sum / total_weight
    combined_variance = 1.0 / total_weight

    combined_exposure = exposure_list[0].clone()
