...
def get_noise_seed(
    *,
    ...
    band : str, optional
        ...
        ``"y"``).  Defaults to ``"i"``.
    survey : str, optional
        Survey name (``"lsst"``, ``"hsc"``, ``"euclid"``).  Mixed into
        the seed so the same physical band in different surveys
        (``lsst_g`` vs ``hsc_g``) draws INDEPENDENT noise.  ``None``
        reproduces the pre-survey seed.
    ...
    int
        ...
        generators.
    """
    # ``survey`` is included only when given, so passing None
    # reproduces the historical (survey-agnostic) seed; galaxy_seed
    # itself is never survey/band dependent (it labels the galaxy,
    # identical across bands/surveys).
