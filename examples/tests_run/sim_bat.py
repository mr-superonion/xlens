...
cfg_sim = IASimConfig()
cfg_sim.include_point_source = False
cfg_sim.draw_image_noise = True
cfg_sim.ia_amplitude = 0.0
...
outdir = os.path.join(
    pscratch,
    f"batsim_constant_shear_{args.layout}",
    test_target,
...
for i in range(istart, iend):
    ...
    expfname = os.path.join(
        outdir, "exposure-%05d.fits" % (sim_seed)
    )
    if not os.path.isfile(expfname):
